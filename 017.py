...
def solution(queryType, query):
    ...
    for i in range(len(queryType)):
        query_name = queryType[i]
        query_value = query[i]
        if query_name == "insert":
            key, value = query_value[0], query_value[1]
            my_dict[key - ck] = value - cv
        elif query_name == "addToValue":
            k = query_value[0]
            cv += k
        elif query_name == "addToKey":
            k = query_value[0]
            ck+=k
        else:
            k = query_value[0]
...
if __name__ == "__main__":
    ...
    result = solution(queryType, query)
    print(f"{result=}")
